...
class Player: # کلاس پلیر های بازی
    ...
    def notification_assign_role(self): # ارسال پیام ابتدای بازی برای هر پلیر
        ...
        if self.role == 'shekar':
            ...
            self.messages = Message(self.create_message('role_Kadkhoda'), self, mid_for_copy=36)
    ...
    def you_were_killed(self,attacker):  # پس از مشخص شدن کشته شدن نقش
        self.alive = False
    ...
    def _kalantar_die(self, attacker):
        self.game.is_vampire_imprisoned = False
...
# پایان حلقه 
    ...
    def ann_remaining_time(self, time):
        ...
        msg = Message(self.dict_game_texts['Join_Message'].format(self.dict_game_texts['Secend'].format(time)), self.group, markup)
    ...
    def silver_spread(self, Ahangar):
        ...
        Message(self.create_message('BlacksmithSpreadSilver'), self.group).send_message()

    def actions_vote(self):
        if self.Ruler_vote != False:
            if self.Ruler_vote.vote != None:
                self.Ruler_vote.vote.you_were_killed(self.Ruler_vote)
    ...
    def analysis_of_events_and_review_of_winning(self):
        ...
        if len(list_Qatel)>0:
            ...
            #                     b[1].alive = False
            #                 Message(self.dict_game_texts['SKHunterEnd'],'hichkas').send_message() # همه باختن
            #                 return True
    ...
    def check_night_target(self): 
        ...
        for player in self.players:
            ...
            elif player.role == 'pishgo': # پیش گو
                if player.target.role == 'Wolfx':
                    player.messages = Message(self.dict_game_texts['Searsee'].format(self.create_tag(player.target), "شاهزاده🤴🏻"), player)  
                elif player.target.role == 'Gorgname':
                    player.messages = Message(self.dict_game_texts['Searsee'].format(self.create_tag(player.target), "گرگینه🐺"), player)
                elif player.target.role == 'Khaen':
                    if random.random() < 0.5:
                        player.messages = Message(self.dict_game_texts['Searsee'].format(self.create_tag(player.target), "روستایی‌ساده👨🏻"), player) 
                    else:
                        player.messages = Message(self.dict_game_texts['Searsee'].format(self.create_tag(player.target), "گرگینه🐺"), player) 
                else:
                    player.messages = Message(self.dict_game_texts['Searsee'].format(self.create_tag(player.target), player.target.role_name), player) 
    ...
    def implementation_action_ngativ(self, ngativ):
        ...
        if ngativ.target != None:
            ...
            if check:
                ...
                ngativ.messages = Message(self.dict_game_texts['NegSeerSees'].format(self.create_tag(random_role)), ngativ)  
            else:
    ...
    def implementation_action_ahmaq(self, ahmaq):
        if ahmaq.target != None:
            ...
            ahmaq.messages = Message(self.dict_game_texts['Searsee'].format(self.create_tag(random_role), random_role.role_name), ahmaq)  
    ...
    def implementation_action_gorg(self,list_gorg,target):
        ...
        if target.alive == True:
            ...
            for ply in self.players:
                if ply.grouping == 'wolf' and ply.role not in ['enchanter','WolfJadogar']:
                    ply.messages = Message(self.create_message('GuardSaved',[target]),ply)
            ...
            if target.becoming_cult != 0: # فرقه میشود یا نه
                ...
                if target.grouping == 'wolf' and target.role not in ['enchanter','WolfJadogar']:
                    if ferqe_caller.alive == True:
                        ...
                        self.group.messages = Message(self.create_message('CultConvertWolfPublic',[target]), self.group)
                    else:
                        pass
        ...
        if Knight.target != None:
            if Knight.target.alive:
                ...
                if Knight.target.grouping == 'village':
                    ...
                    self.group.messages = Message(self.create_message('KnightKillPlayerGroupMessage').format(self.create_tag(Knight), self.roleannouncement(Knight.target)), self.group)
    ...
    def chack_number_player(self):
        ...
        if number_players>=3:  # چک برای اجرای بازی با تعداد کافی
